...
This example demonstrates how to use the function to perform IQR filtering on specified variables in a data frame and update an
...
```{r}
user_class$se('NEE', uStarTh = uStarThAnnual, uStarSuffixes =  uStarSuffixes, FillAll = TRUE)

grep("NEE_.*_f$",names(Bartlett.2004.2022.IQR$sExportResults())
     , value = TRUE)
grep("NEE_.*_fsd$",names(Bartlett.2004.2022.IQR$sExportResults())
     , value = TRUE)
```
